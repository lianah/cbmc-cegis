/* FUNCTION: __CPROVER_danger_execute */

#define __CPROVER_danger_number_of_ops 2
#define __CPROVER_danger_number_of_consts 1
#define __CPROVER_danger_number_of_vars 2
#define __CPROVER_danger_max_solution_size 1

const void *__CPROVER_danger_OPS[__CPROVER_danger_number_of_ops ];
void *__CPROVER_danger_RESULT_OPS[__CPROVER_danger_max_solution_size ];

typedef unsigned char opcodet;
typedef unsigned char opt;
struct __CPROVER_danger_instructiont
{
  opcodet opcode;
  opt op0;
  opt op1;
  opt op2;
};

#define __CPROVER_danger_max_instruction 24u

void __CPROVER_danger_execute(struct __CPROVER_danger_instructiont *program,
    unsigned char size)
{
  for (unsigned char i=0; i < size; ++i)
  {
#define opcode program[i].opcode
    __CPROVER_assume(opcode<= __CPROVER_danger_max_instruction);
#define op0_id program[i].op0
#define op1_id program[i].op1
#define op2_id program[i].op2
#define max_op_index (__CPROVER_danger_number_of_vars + i)
    __CPROVER_assume(op0_id < max_op_index && op1_id < max_op_index && op2_id < max_op_index
    && (op0_id >= __CPROVER_danger_number_of_consts || op1_id >= __CPROVER_danger_number_of_consts || op2_id >= __CPROVER_danger_number_of_consts)
    && (opcode > 5u || op0_id <= op1_id) && (opcode < 21u || !op1_id)
    && (opcode == 9u || opcode >= 15u && opcode <= 18 || !op2_id)
    && (opcode != 9u || op0_id != op2_id || op1_id <= op2_id));
    const unsigned int * const op0_ptr=__CPROVER_danger_OPS[op0_id];
    const unsigned int * const op1_ptr=__CPROVER_danger_OPS[op1_id];
    const unsigned int * const op2_ptr=__CPROVER_danger_OPS[op2_id];
    __CPROVER_assume(op0_ptr && op1_ptr && op2_ptr); // No null pointers in op array
    const unsigned int op0=*op0_ptr;
    const unsigned int op1=*op1_ptr;
    __CPROVER_assume(opcode != 19 && opcode != 20 || op1);// Avoid div by 0.
    const unsigned int op2=*op2_ptr;
#define sop0 ((int) op0)
#define sop1 ((int) op1)
#define sop2 ((int) op2)

    unsigned int result;
    if (opcode < 15)
    if (opcode < 7)
    if (opcode < 3)
    if (opcode < 1)
    __CPROVER_danger_opcode_0: result=op0 + op1;
    else if (opcode < 2)
    __CPROVER_danger_opcode_1: result=op0 * op1;
    else
    __CPROVER_danger_opcode_2: result=op0 & op1;
    else
    if (opcode < 5)
    if (opcode < 4)
    __CPROVER_danger_opcode_3: result=op0 | op1;
    else
    __CPROVER_danger_opcode_4: result=op0 ^ op1;
    else if (opcode < 6)
    __CPROVER_danger_opcode_5: result=op0 != op1;
    else
    __CPROVER_danger_opcode_6: result=!op0 || op1;
    else
    if (opcode < 11)
    if (opcode < 9)
    if (opcode < 8)
    {
      __CPROVER_danger_opcode_first_7: result=op0 < op1;
      if (result) result=op0;
      else __CPROVER_danger_opcode_last_7: result=op1;
    }
    else
    {
      __CPROVER_danger_opcode_first_8: result=op0 > op1;
      if (result) result=op0;
      else __CPROVER_danger_opcode_last_8: result=op1;
    }
    else if (opcode < 10)
    {
      __CPROVER_danger_opcode_first_9: if (op0) result=op1;
      else __CPROVER_danger_opcode_last_9: result=op2;
    }
    else
    __CPROVER_danger_opcode_10: result=op0 - op1;
    else
    if (opcode < 13)
    if (opcode < 12)
    __CPROVER_danger_opcode_11: result=op0 << op1;
    else
    __CPROVER_danger_opcode_12: result=op0 >> op1;
    else if (opcode < 14)
    __CPROVER_danger_opcode_13: result=op0 >> op1;
    else
    __CPROVER_danger_opcode_14: result=sop0 >> op1;
    else if (opcode < 19)
    if (opcode < 17)
    if (opcode < 16)
    __CPROVER_danger_opcode_15: result=op0 <= op1 && op1 <= op2;
    else
    __CPROVER_danger_opcode_16: result=op0 < op1 && op1 < op2;
    else if (opcode < 18)
    __CPROVER_danger_opcode_17: result=sop0 <= sop1 && sop1 <= sop2;
    else
    __CPROVER_danger_opcode_18: result=sop0 < sop1 && sop1 < sop2;
    else if (opcode < 23)
    if (opcode < 21)
    if (opcode < 20)
    __CPROVER_danger_opcode_19: result=op0 / op1;
    else
    __CPROVER_danger_opcode_20: result=op0 % op1;
    else if (opcode < 22)
    __CPROVER_danger_opcode_21: result=-op0;
    else
    __CPROVER_danger_opcode_22: result=~op0;
    else if (opcode < 24)
    //__CPROVER_danger_opcode_23: result=0u;
    __CPROVER_danger_opcode_23: result=sop0 == -1;
    else
    __CPROVER_danger_opcode_24: result=op0;
    //__CPROVER_danger_opcode_24: result=sop0 != -1;

    *(unsigned int *)__CPROVER_danger_RESULT_OPS[i]=result;
  }
}

int main(void)
{
  unsigned int x=16;
  const unsigned int constant=17;
  __CPROVER_danger_OPS[0]=&constant;
  __CPROVER_danger_OPS[1]=&x;
  unsigned int result=1;
  __CPROVER_danger_RESULT_OPS[0]=&result;
#undef opcode
  //struct __CPROVER_danger_instructiont instr = { .opcode=0u, .op0=0u, .op1=1u, .op2=0u };
  struct __CPROVER_danger_instructiont instr = { 15u, 0u, 1u, 0u };
  __CPROVER_danger_execute(&instr, 1);
  //__CPROVER_assert(0 == 1, "");
  __CPROVER_assert(result != 0, "");
  return 0;
}
