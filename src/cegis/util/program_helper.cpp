...
goto_programt &get_synth_body(goto_functionst &gf)
{
  
  return get_danger_body(gf);
}

const goto_programt &get_synth_body(const goto_functionst &gf)
{
  return get_danger_body(gf);
}

bool is_synth_user_variable(const irep_idt &id, const typet &type) {
  return is_danger_user_variable(id, type);
}
