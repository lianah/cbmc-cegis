...
namespace
{
const char ROP[]="__CPROVER_synth_execute::1::1::result";
const char OP0[]="__CPROVER_synth_execute::1::1::op0";
const char OP1[]="__CPROVER_synth_execute::1::1::op1";
const char OP2[]="__CPROVER_synth_execute::1::1::op2";
...
private:
  ...
  const synth_variable_namest &rnames;
  const size_t op0;
  const size_t op1;
  const size_t op2;
...
public:
  ...
  {
    typedef synth_variable_namest::const_iterator itt;
...
public:
  virtual void operator()(exprt &expr)
  {
    ...
    const synth_variable_namest &names=is_res ? rnames : this->names;
    const size_t op=is_res ? instr_idx : is_op0 ? op0 : is_op1 ? op1 : op2;
    const synth_variable_namest::const_iterator name=names.find(op);
    if (names.end() == name) return;
    const symbol_exprt symbol(st.lookup(name->second).symbol_expr());
    const typet synth_type(synth_meta_type());
    if (type_eq(synth_type, symbol.type(), ns)) expr=symbol;
    else expr=typecast_exprt(symbol, synth_type); // XXX: Change if operations for other types are added.
  }
};
}

void Synth::replace_ops_in_instr(const symbol_tablet &st,
