...
const char X_CHOICE_PREFIX[]=SYNTH_PREFIX"x_choice_";
    ...
    goto_programt::targett pos, const array_valuest &vals)
{
  for (array_valuest::const_iterator it=vals.begin(); it != vals.end(); ++it)
  {
    ...
    pos=declare_synth_variable(st, gf, pos, base_name, type);
    pos=assign_synth_variable(st, gf, pos, base_name, value);
  }
}

const char X_LABEL[]=SYNTH_PREFIX"x_loop";
const char X_INDEX[]=SYNTH_PREFIX"x_index";
goto_programt::targett add_ce_loop(synth_programt &prog, const size_t ces_size)
{
  symbol_tablet &st=prog.st;
  goto_functionst &gf=prog.gf;
  goto_programt::targett pos=prog.synth_range.begin;
  const typet size_type(unsigned_int_type());
  pos=declare_synth_variable(st, gf, --pos, X_INDEX, size_type);
  const constant_exprt first_index(from_integer(0, size_type));
  pos=assign_synth_variable(st, gf, pos, X_INDEX, first_index);
  goto_programt::targett loop_head=pos;
  (++loop_head)->labels.push_back(X_LABEL);
  goto_programt &body=get_synth_body(gf);
  pos=body.insert_before(prog.synth_range.end);
  pos->type=goto_program_instruction_typet::ASSIGN;
  pos->source_location=default_synth_source_location();
  const std::string index_name(get_synth_meta_name(X_INDEX));
...
public:
  assign_ce_valuet(synth_programt &prog) :
  ...
  void operator()(const std::pair<const irep_idt, exprt> &assignment)
  {
    ...
    const std::string array_name(get_synth_meta_name(base_name));
    ...
    pos=synth_assign(st, gf, pos, lhs, rhs);
  }
};

void assign_ce_values(synth_programt &prog, const counterexamplet &ce)
{
  const assign_ce_valuet assign_value(prog);
  std::for_each(ce.begin(), ce.end(), assign_value);
}

const char CONSTRAINTS[]=SYNTH_PREFIX"constraints";
void create_constraints(synth_programt &prog)
{
    ...
    const goto_programt::targett &loop_end)
{
  ...
  assertion->source_location=default_synth_source_location();
  assertion->guard=false_exprt();
}
}

void Synth::synth_add_learned_counterexamples(synth_programt &prog,
