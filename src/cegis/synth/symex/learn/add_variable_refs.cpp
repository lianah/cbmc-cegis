    ...
    const char * const ops_array, const exprt &rhs, const unsigned int id)
{
  ...
  set_op.type=ASSIGN;
  set_op.source_location=default_synth_source_location();
...
{
  return set_ops_reference(st, body, pos, SYNTH_OPS, rhs, id);
}
...
{
  return set_ops_reference(st, body, pos, SYNTH_OPS, name, id);
}
...
void link_user_symbols(const symbol_tablet &st, synth_variable_idst &var_ids,
    size_t &variable_id, bool consts)
{
  ...
  for (symbolst::const_iterator it=symbols.begin(); it != symbols.end(); ++it)
  {
    ...
    if (!is_synth_user_variable(symbol.name, symbol.type)) continue;
...
void link_user_program_variables(synth_programt &prog,
    const synth_variable_idst &var_ids)
{
  synth_variable_idst to_instrument(var_ids);
  goto_programt &body=get_synth_body(prog.gf);
  goto_programt::instructionst &instrs=body.instructions;
  const goto_programt::targett end=prog.synth_range.end;
  ...
  for (goto_programt::targett it=instrs.begin(); it != end; ++it)
  {
    ...
    if (!is_synth_user_variable(name, typet())) continue;
    const synth_variable_idst::const_iterator id=var_ids.find(name);
    switch (type)
    {
  ...
  goto_programt::targett pos=pred(prog.synth_range.begin);
  synth_variable_idst::const_iterator it;
...
goto_programt::targett set_rops_reference(const symbol_tablet &st,
    ...
    const irep_idt &name, const unsigned int id)
{
  return set_ops_reference(st, body, pos, SYNTH_RESULT_OPS, name, id);
}
    ...
    const size_t num_user_vars)
{
  ...
  {
    const std::string name=get_synth_meta_name(get_tmp(i));
...
public:
  link_single_resultt(const symbol_tablet &st, goto_functionst &gf,
      ...
      st(st), body(get_synth_body(gf)), num_user_vars(num_user_vars), max_solution_size(
          max_solution_size)
  {
  }
...
public:
  link_meta_variablest(synth_programt &prog, const size_t num_user_vars,
      const size_t max_solution_size) :
      prog(prog), user_vars(num_user_vars), max_size(max_solution_size)
  {
  }

  void operator()(const synth_programt::loopt &loop) const
  {
...
void link_meta_variables(synth_programt &prog, const size_t user_vars,
    const size_t max_solution_size)
{
  const symbol_tablet &st=prog.st;
...
void Synth::synth_add_variable_refs(synth_programt &prog,
...
size_t Synth::get_synth_variable_ids(const symbol_tablet &st,
