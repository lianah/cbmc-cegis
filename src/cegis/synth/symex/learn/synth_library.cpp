...
namespace
{
const char PROGRAM_ARG_NAME[]="__CPROVER_synth_execute::program";
const char PROGRAM_ARG_BASE_NAME[]="program";
const char SIZE_ARG_NAME[]="__CPROVER_synth_execute::size";
const char SIZE_ARG_BASE_NAME[]="size";

pointer_typet instr_type()
{
  return pointer_typet(symbol_typet(SYNTH_INSTRUCTION_TYPE_NAME));
}

void add_placeholder(symbol_tablet &symbol_table)
{
  if (symbol_table.has_symbol(SYNTH_EXECUTE)) return;
  symbolt symbol;
  symbol.name=SYNTH_EXECUTE;
...
void set_loop_id(goto_functionst &gf)
{
  ...
  goto_functionst::function_mapt::iterator execute=fm.find(SYNTH_EXECUTE);
...
void set_init_values(synth_programt &prog)
{
  goto_programt &body=get_synth_body(prog.gf);
  goto_programt::targett pos=prog.synth_range.begin;
  const symbol_tablet &st=prog.st;
  pos=init_array(st, body, SYNTH_OPS, --pos);
  pos=init_array(st, body, SYNTH_RESULT_OPS, pos);
}
}

void Synth::add_synth_library(synth_programt &prog, message_handlert &msg,
    ...
    const size_t max_solution_size)
{
  ...
  functions.insert(SYNTH_EXECUTE);
  std::string prefix("#define __CPROVER_synth_number_of_vars ");
  prefix+=integer2string(num_vars);
  prefix+="\n#define __CPROVER_synth_number_of_consts ";
  prefix+=integer2string(num_consts);
  prefix+="\n#define __CPROVER_synth_number_of_ops ";
  prefix+=integer2string(num_vars + max_solution_size - 1);
  prefix+="\n#define __CPROVER_synth_max_solution_size ";
  prefix+=integer2string(max_solution_size);
  prefix+='\n';
  add_cprover_library(functions, symbol_table, msg, prefix);
  goto_convert(SYNTH_EXECUTE, symbol_table, goto_functions, msg);
