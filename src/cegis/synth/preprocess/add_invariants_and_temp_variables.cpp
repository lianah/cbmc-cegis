...
void createIx0(synth_programt &prog)
{
  ...
  meta=declare_synth_variable(prog.st, prog.gf, --pos, get_Ix0(), type);
}
...
public:
  create_meta_variables_for_loopt(synth_programt &prog) :
    st(prog.st), gf(prog.gf), loop_id(0u), rank(prog.synth_ranking)
  {
  }
  ...
  void operator()(synth_programt::loopt &loop)
  {
    ...
    const std::string inv(get_Ix(loop_id));
    meta.Ix=declare_synth_variable(st, gf, --pos, inv, type);
    const std::string guard(get_Gx(loop_id));
    meta.Gx=declare_synth_variable(st, gf, meta.Ix, guard, type);
    ...
    pos=loop.body.end;
    const std::string x_prime(get_Ix_prime(loop_id));
    meta.Ix_prime=declare_synth_variable(st, gf, --pos, x_prime, type);
    pos=meta.Ix_prime;
