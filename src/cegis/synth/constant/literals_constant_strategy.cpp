...
class compare_constantt
{
  const namespacet ns;
public:
  compare_constantt(const synth_programt &program) :
      ns(program.st)
  {
  }
...
public:
  ...
  constant_expr_visitort(const synth_programt &prog, constant_sett &constants) :
      ns(prog.st), type(synth_meta_type()), constants(constants)
  {
...
public:
  add_constantt(synth_programt &program) :
      program(program)
  {
  }
  ...
  {
    add_synth_constant(program, expr);
    // XXX: Add negation of every constant?
    // if (!expr.is_zero()) add_synth_constant(program, unary_minus_exprt(expr));
  }
};
}

size_t Synth::literals_constant_strategy(Synth::synth_programt &program,
					 const size_t max_length)
{
  ...
  const synth_programt::program_ranget &range=program.synth_range;
