...
std::string Synth::get_Ax()
{
  return SYNTH_PREFIX"A_x";
}
...
namespace
{
const char GUARD_PREFIX[]=SYNTH_PREFIX"G";
}

std::string Synth::get_Gx(const size_t loop_id)
{
...
const char RANKING_PREFIX[]=SYNTH_PREFIX"R";
}

std::string Synth::get_Rx(const size_t loop_id, const size_t result_id)
{
  return build_var_name(RANKING_PREFIX, loop_id, STATE_BEFORE_LOOP, result_id);
}

std::string Synth::get_Rx_prime(const size_t loop_id, const size_t result_id)
{
...
{
  std::string result(SYNTH_TMP_PREFIX);
