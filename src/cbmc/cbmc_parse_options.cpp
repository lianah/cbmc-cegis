...
int cbmc_parse_optionst::doit()
{
  ...
  if(cmdline.isset("danger"))
  {
    ...
    const Danger::constant_strategyt strategy=Danger::default_constant_strategy;
    Danger::danger_preprocessingt preproc(symbol_table, goto_functions, strategy);

    const Danger::danger_programt &prog=preproc.get_danger_program();
    Danger::danger_verify_configt verify_config(prog);
    cegis_symex_verifyt<Danger::danger_verify_configt> verify(options, verify_config);
    Danger::danger_learn_configt learn_config(prog);
    cegis_symex_learnt<Danger::danger_learn_configt> learn(options, learn_config);
